test_that("psao() on 5-component vertices", {
  ...
  X[11:15, 5] <- 1
  colSums(X)
  # since 1s become 1s after a merge. I expect PSSA to get pairs
  # 1,2 (weight of 1/3 ish)
  # 1,2 (weight of 1/2 ish)
  # 2,3 (weight of 4/9 ish)
  # 1,2 (weight of 9/15 ish)
  out <- suppressMessages(psao(X, testweights = seq(0, 1, length.out = 100)))
  ...
  expect_equal(out$merges[3, c("v1", "v2")] |> unlist(), c(1, 2), ignore_attr = TRUE)
...
test_that("psao() can be applied using provided merges", {
  # 1,2,3,4,5
  X <- matrix(0, ncol = 5, nrow = 15)
  X[1, 1] <- 1
  X[2:3, 2] <- 1
  ...
  out <- suppressMessages(psao(X, merges = merges))
  expect_equal(out$merges, merges, ignore_attr = TRUE)

  expect_true(all(round(abs(do.call(cbind, out$scores[-1])), 10) %in% round(c(pi/4, 0), 10))) #pi/4 because data points are vertices and the merges occur at halfway
})
